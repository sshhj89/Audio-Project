...
void loop() {  
  ...
  { 
    ...
    countForwardButton = 0;
      char name[10];
     int mok = currentFile/10;
     int remain = currentFile % 10;
     if( mok < 1)
     {
       name[0] = remain + 48;
       name[1]='.';
     ...
     {
         int i=0;
          int tempNum = currentFile;
          name[mok] = tempNum%10 + 48;   
          tempNum/=10;
         // Serial.print("mok: "); Serial.println(mok);
         for(i=0;i<mok;i++)
         {
            name[mok-i-1] = tempNum + 48;
            tempNum/=10;
         }
         name[i+1]='.';
         name[i+2]='o';
    ...
    if (! musicPlayer.startPlayingFile(name)  ) {
    ...
    if(digitalRead(REC_BUTTON) == LOW)
    {
     ...
     {  
       ...
       while(!digitalRead(FORWARD_BUTTON))
       {
             ...
             {
                  Serial.println("FF");
                  if(musicPlayer.currentTrack.position() <= 0)
                  {  
